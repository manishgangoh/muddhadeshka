...
export default async function ArticlePage({ params, searchParams }) {
  ...
  // The full original (multi-source merged) article is generated by AI, which is slow
  // (scrape + LLM). NEVER block the page on it — show the source summary instantly and
  // build the full article in the BACKGROUND. AutoRefresh reloads once it's ready.
  const pending = !a.full_content;
  if (pending) {
    after(async () => { try { await buildArticleContent(a); } catch { /* retried on next visit */ } });
  }
